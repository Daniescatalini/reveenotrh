...
const ACCOUNT_CREATED_MONTH = "2026-05";
...
const colorChoices = ["#d75c27", "#211d19", "#2f80ed", "#27ae60", "#f2a93b", "#9b6bdf", "#e74c3c", "#8a7c72"];
...
function CategoryBadgeIcon({
  ...
  if (logoUrl) {
    return <img src={logoUrl} alt="" className="h-10 w-10 rounded-2xl object-cover" />;
  }

  return (
    <span
      className="flex h-10 w-10 items-center justify-center rounded-2xl text-white"
      style={{ background: category?.color ?? "#d75c27" }}
    >
...
function buildMonthOptions(selectedMonth: string) {
  const [year, month] = ACCOUNT_CREATED_MONTH.split("-").map(Number);
...
function buildMetrics(data: MonthData) {
  ...
  const pendingBills = data.bills.filter((bill) => bill.status === "pendente");
  const overdueBills = data.bills.filter((bill) => bill.status === "atrasada");
...
function calculateNorthScore(metrics: ReturnType<typeof buildMetrics>, goals: Goal[]) {
  const paidOnTime = metrics.paidBills.length * 8;
  const overduePenalty = metrics.overdueBills.length * 12;
  const reserveProgress = Math.min(20, Math.round(((goals[0]?.current ?? 0) / (goals[0]?.target ?? 1)) * 20));
  const positiveBalance = metrics.unassignedValue >= 0 ? 15 : 0;
  return Math.max(0, Math.min(100, 55 + paidOnTime + reserveProgress + positiveBalance - overduePenalty));
}
        ...
        <div className="relative mt-auto shrink-0 pt-3">
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="truncate text-[11px] text-white/50">{user.plan}</p>
            </div>
            ) : null}
            {!collapsed ? (
            <ChevronDown
...
function MonthFilter({
  ...
  const options = buildMonthOptions(selectedMonth);
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-[var(--muted)]">
                    {formatCurrency(bill.expectedAmount)} • vence {formatDate(bill.dueDate)}
                  </p>
                </div>
                <button
                  type="button"
...
function buildFinanceDetail(
  ...
  const pendingItems = [...metrics.pendingBills, ...metrics.overdueBills].map((bill) => ({
    date: bill.dueDate,
    label: bill.name,
    helper: bill.status === "atrasada" ? "Conta atrasada" : "Conta pendente",
  ...
  if (label === "Contas atrasadas") {
    return {
      ...
      sections: [{ title: "Atrasadas no período", total: metrics.totalOverdue, items: pendingItems.filter((item) => item.helper === "Conta atrasada") }],
    };
  }
...
function ReportsView({
  ...
  const reserveProgress = Math.round(((data.goals[0]?.current ?? 0) / Math.max(data.goals[0]?.target ?? 1, 1)) * 100);
  const completedObjectives = data.objectives.filter((objective) => objective.done).length;
  const totalObjectives = Math.max(data.objectives.length, 1);
  const moneyDestinationTotal = Math.max(sum(data.bills, (bill) => bill.paidAmount ?? bill.expectedAmount) + metrics.reserveDestination + 300, 1);
  const categoryTotals = categories
    .filter((category) => category.type === "conta")
    .map((category) => ({
      label: category.name,
      value: sum(data.bills.filter((bill) => bill.category === category.name), (bill) => bill.paidAmount ?? bill.expectedAmount),
      color: category.color,
    }))
    .filter((item) => item.value > 0)
    .sort((a, b) => b.value - a.value);
  const distributionItems = [
    { label: "Moradia", value: categoryTotals.find((item) => item.label === "Moradia")?.value ?? 0, color: "#f06a32" },
    { label: "Alimentação", value: categoryTotals.find((item) => ["Alimentacao", "Alimentação"].includes(item.label))?.value ?? 0, color: "#31b878" },
    { label: "Dívidas", value: categoryTotals.find((item) => ["Dividas", "Dívidas"].includes(item.label))?.value ?? 0, color: "#f4b840" },
    { label: "Metas e Reserva", value: metrics.reserveDestination + 300, color: "#9b6bdf" },
  ].filter((item) => item.value > 0);
  ...
  const trendRows = [
    { label: "Jan", income: 2400, out: 1200 },
    { label: "Fev", income: 3600, out: 2300 },
    { label: "Mar", income: 5200, out: 3200 },
    { label: "Abr", income: Math.max(metrics.totalIncome - 1800, 2900), out: Math.max(metrics.totalPaid + 650, 1800) },
    { label: "Mai", income: Math.max(metrics.totalIncome - 420, 4100), out: Math.max(metrics.totalPaid + 2200, 2800) },
    { label: reportMonth.slice(0, 3), income: metrics.totalIncome, out: metrics.totalPaid + metrics.totalPending + metrics.totalOverdue },
  ];
  ...
  const timelineItems = [
    data.incomes[0] ? { date: data.incomes[0].receivedDate, icon: Wallet, title: "Recebeu salário", detail: `+ ${formatCurrency(data.incomes[0].amount)}`, tone: "green" } : null,
    metrics.paidBills[0] ? { date: metrics.paidBills[0].paidDate ?? metrics.paidBills[0].dueDate, icon: ReceiptText, title: `Quitou ${metrics.paidBills[0].name}`, detail: `- ${formatCurrency(metrics.paidBills[0].paidAmount ?? metrics.paidBills[0].expectedAmount)}`, tone: "orange" } : null,
    data.goals[0] ? { date: `${data.selectedMonth}-15`, icon: Flag, title: `Criou meta ${data.goals[0].name.split(" ")[0]}`, detail: `Meta de ${formatCurrency(data.goals[0].target)}`, tone: "purple" } : null,
    { date: `${data.selectedMonth}-18`, icon: PiggyBank, title: "Iniciou reserva", detail: `+ ${formatCurrency(metrics.reserveDestination || 250)} guardados`, tone: "green" },
    { date: `${data.selectedMonth}-28`, icon: Sparkles, title: "North Score atingiu 85", detail: "+7 pontos", tone: "orange" },
    { date: `${data.selectedMonth}-30`, icon: Flag, title: "Fim do mês", detail: `Saldo final ${formatCurrency(metrics.unassignedValue)}`, tone: "dark" },
  ].filter(Boolean) as Array<{ date: string; icon: React.ElementType; title: string; detail: string; tone: string }>;
  const diagnostic = [
    { icon: Target, label: "Organização", helper: "Compromissos em dia e planejamento", value: 85, status: "Excelente", color: "#31b878" },
    { icon: Wallet, label: "Controle de gastos", helper: "Gastos dentro do planejado", value: 72, status: "Bom", color: "#2f80ed" },
    ...
    { icon: Zap, label: "Dívidas", helper: "Pendências e contas atrasadas", value: metrics.overdueBills.length ? 40 : 88, status: metrics.overdueBills.length ? "Prioridade alta" : "Sob controle", color: "#ef4e3e" },
  ];
  const insights = [
    { icon: ArrowUpRight, title: "Melhor mês do semestre", helper: "Seu saldo foi o maior dos últimos 6 meses.", tone: "green" },
    { icon: TrendingUp, title: `Receita cresceu ${Math.abs(metrics.incomeDelta || 14)}%`, helper: `Comparado a ${previousMonthName}.`, tone: "green" },
    { icon: ArrowUpRight, title: "Gastos cresceram 8%", helper: "Fique atento aos gastos com lazer e delivery.", tone: "orange" },
  ];
  const recommendations = [
    { title: "Quitar conta Energia", helper: "Impacto: +4 pontos no North Score", action: "Contas atrasadas" },
    { title: "Aumentar reserva para R$ 500", helper: "Impacto: mais segurança e menos ansiedade", action: "Reserva" },
    { title: "Reduzir lazer em R$ 100", helper: "Impacto: antecipar meta em 1 mês", action: "Livre para decidir" },
  ];
    ...
    <div className="space-y-4">
      <section className="relative overflow-hidden rounded-[28px] border border-white/12 bg-[radial-gradient(circle_at_92%_16%,rgba(215,92,39,.44),transparent_20rem),linear-gradient(135deg,#211d19,#171411)] p-4 text-white shadow-[0_18px_50px_rgba(33,29,25,.16)] sm:p-5 md:p-7">
        ...
        <div className="relative grid gap-4 lg:grid-cols-[1fr_260px] lg:items-center">
          <div className="flex gap-3 sm:gap-5">
            ...
            <div>
              <h3 className="text-[1.35rem] font-extrabold leading-tight tracking-tight sm:text-2xl">{reportMonth} foi um mês positivo!</h3>
              <p className="mt-2 max-w-2xl text-[0.82rem] font-medium leading-5 text-white/76 sm:mt-3 sm:text-sm sm:font-semibold sm:leading-6">
                Seu saldo final cresceu {Math.abs(metrics.incomeDelta || 14)}%. Você pagou {paidCount} conta(s), iniciou sua reserva financeira e reduziu suas pendências em {formatCurrency(420)}.
              </p>
              <button
                type="button"
                onClick={() => onOpenFinanceDetail(buildFinanceDetail("Contas atrasadas", data, metrics, realBalance))}
              ...
              >
                Seu principal desafio continua sendo as contas atrasadas.
              </button>
            </div>
          </div>
          <button
          ...
          >
            ...
            <p className="mt-1 text-[11px] font-bold text-emerald-300 sm:mt-2 sm:text-xs">↑ +7 pontos em relação a {previousMonthName}</p>
          </button>
        </div>
      </section>
      ...
      <div className="grid gap-4 xl:grid-cols-[1.05fr_1fr_1fr]">
        ...
        <Card className="p-5">
          ...
          <button type="button" className="mt-5 flex w-full items-center justify-between rounded-2xl border border-[var(--line)] px-4 py-3 text-xs font-extrabold">
            Ver diagnóstico completo <ChevronRight className="h-4 w-4" />
          </button>
        </Card>
        ...
        <Card className="p-5">
          ...
          <div className="mt-5 overflow-x-auto pb-1">
            <div className="relative flex min-w-[720px] items-start justify-between gap-3 px-3">
              ...
              {timelineItems.map(({ date, icon: Icon, title, detail, tone }) => (
                <button
                  type="button"
                  key={`${date}-${title}`}
                  onClick={() => onOpenFinanceDetail(buildFinanceDetail(title, data, metrics, realBalance))}
                ...
                <Question eyebrow="Despesas" title="Quais são suas despesas fixas mensais?" subtitle="Não se preocupe em lembrar tudo, a North te ajuda.">
                  <div className="max-w-3xl space-y-3">
                    {answers.fixedExpenses.length ? answers.fixedExpenses.map((expense) => (
                      ...
                      <p className="rounded-2xl border border-dashed border-[#211d19]/14 bg-white/44 p-4 text-sm font-bold text-[#756b62]">
                        Você ainda não adicionou nenhuma despesa fixa.
                      </p>
                    )}
                    <div className="grid items-end gap-3 rounded-3xl border border-[#211d19]/8 bg-white/54 p-3 shadow-[0_14px_42px_rgba(33,29,25,0.05)] md:grid-cols-[1fr_180px_auto]">
                      ...
                      <button type="button" onClick={addFixedExpense} className="h-12 rounded-2xl bg-[#211d19] px-5 text-sm font-extrabold text-white">Adicionar</button>
                    </div>
                  </div>
                </Question>
              ) : null}
...
function CompactRow({
  ...
  return (
    <div className="flex items-center gap-3 rounded-2xl border border-[#211d19]/8 bg-white/72 px-3 py-3 shadow-[0_10px_26px_rgba(33,29,25,0.04)]">
      ...
      <MoreVertical className="h-4 w-4 text-[#756b62]" />
    </div>
  );
}
...
function AccountModal({
  ...
  const currentCategory = categories.find((category) => category.name === draft.category);
  ...
  return (
    <Modal title="Editar conta" onClose={onClose}>
      <div className="grid gap-4 md:grid-cols-[auto_1fr] md:items-start">
        <label className="flex cursor-pointer items-center gap-3 rounded-3xl border border-[var(--line)] bg-white/35 p-4 dark:bg-white/5">
          <input
            type="file"
            accept="image/*"
            className="hidden"
          ...
          />
          {draft.logoUrl ? (
            <img src={draft.logoUrl} alt="" className="h-12 w-12 rounded-2xl object-cover" />
          ) : (
            <span
              className="flex h-12 w-12 items-center justify-center rounded-2xl text-white"
              style={{ background: currentCategory?.color ?? "#d75c27" }}
            >
              <CurrentIcon className="h-5 w-5" />
            </span>
          )}
          <div>
            <p className="text-sm font-extrabold">{draft.category}</p>
            <p className="text-[11px] font-semibold text-[var(--muted)]">
              {draft.logoUrl ? "Clique para trocar logo" : "Clique para enviar logo"}
            </p>
          </div>
        </label>
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2">
        <label className="flex items-center justify-between rounded-2xl border border-[var(--line)] bg-white/35 p-4 dark:bg-white/5">
        ...
        <div className="grid gap-4">
          ...
          <label className="block">
            ...
            <select
              ...
              className="mt-2 w-full rounded-2xl border border-[var(--line)] bg-white/55 px-4 py-3 text-sm font-semibold outline-none focus:border-[#d75c27] dark:bg-white/8"
            >
              {categories.filter((category) => category.type === "conta").map((category) => (
  ...
  return (
    <Modal title={goal.id ? "Detalhes da meta" : "Nova meta"} onClose={onClose}>
      ...
      <div className="mt-5 rounded-3xl border border-[var(--line)] bg-white/35 p-4 dark:bg-white/5">
        ...
        <p className="mt-3 text-xs font-semibold text-[var(--muted)]">
          Restam {formatCurrency(remaining)}. Previsão: {formatDate(draft.deadline)}.
        </p>
      </div>
      <div className="mt-5 flex flex-wrap justify-between gap-3">
        <button
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <div className="relative">
          ...
          <button
            ...
            className="mt-2 flex h-12 w-12 items-center justify-center rounded-2xl text-white"
        ...
        <label className="relative block">
          ...
          <button
            ...
            className="mt-2 block h-9 w-9 rounded-full border-2 border-white shadow-[0_0_0_1px_rgba(33,29,25,.08)]"
            style={{ background: draft.color }}
...
function NorthIADrawer({
  ...
  const answers: Record<string, string> = {
    ...
    "Qual conta priorizar?": "Priorize a conta atrasada ou essencial mais próxima do vencimento. Depois, direcione uma parte para futuro e prioridades.",
        ...
        {categories
          ...
          .map((category) => {
            ...
            return (
              <div
                key={category.id}
                className="grid items-center gap-3 rounded-3xl border border-[var(--line)] bg-white/35 p-3 dark:bg-white/5 md:grid-cols-[auto_minmax(180px,1fr)_auto_auto_auto]"
              >
              <div className="relative">
                <button
                  type="button"
                ...
                <div className="relative">
                  <button
                    ...
                    className="h-7 w-7 rounded-full border-2 border-white shadow-[0_0_0_1px_rgba(33,29,25,.08)]"
                    style={{ background: category.color }}
                ...
                <button
                  ...
                  className="rounded-2xl border border-red-500/20 px-3 py-2 text-xs font-extrabold text-red-600"
                >
                  Excluir
                </button>
              </div>
...
function IconPicker({ onSelect }: { onSelect: (icon: string) => void }) {
  return (
    <div className="absolute left-0 top-13 z-20 grid w-64 grid-cols-4 gap-2 rounded-3xl border border-[var(--line)] bg-white/95 p-3 shadow-2xl backdrop-blur-xl dark:bg-[#211d19]/95">
      {visualIconChoices.map(({ id, Icon }) => (
        <button
          type="button"
          key={id}
          onClick={() => onSelect(id)}
          className="flex h-11 w-11 items-center justify-center rounded-2xl bg-[#211d19]/6 text-[#211d19] transition hover:bg-[#d75c27] hover:text-white dark:bg-white/8 dark:text-white"
...
function ColorPickerPopover({ value, onChange }: { value: string; onChange: (color: string) => void }) {
  ...
  return (
    <div className="absolute right-0 top-9 z-20 w-64 rounded-3xl border border-[var(--line)] bg-white/95 p-3 shadow-2xl backdrop-blur-xl dark:bg-[#211d19]/95">
      <div className="grid grid-cols-4 gap-2">
        {colorChoices.map((color) => (
          <button
            key={color}
      ...
      <label className="mt-3 block">
        ...
        <div className="mt-2 flex gap-2">
          <input
            ...
            placeholder="#d75c27"
            className="min-w-0 flex-1 rounded-2xl border border-[var(--line)] bg-white/60 px-3 py-2 text-xs font-bold outline-none dark:bg-white/8"
          />
          <button
            type="button"
            onClick={() => /^#[0-9a-fA-F]{6}$/.test(custom) && onChange(custom)}
            className="rounded-2xl bg-[#211d19] px-3 py-2 text-[11px] font-extrabold text-white dark:bg-[#d75c27]"
          >
            Aplicar
          </button>
        </div>
...
function DataPanel({
  ...
  const options = buildMonthOptions(ACCOUNT_CREATED_MONTH);
  ...
  const filteredBills = bills.filter((bill) => inRange(bill.dueDate));
  ...
  const saved = Math.min(received, 550);
  ...
  const northScore = Math.max(0, Math.min(100, 82 + paidBills.length - overdueBills.length * 4));
      ...
      <html>
        ...
        <body>
          <main class="page">
            ...
            <section class="hero">
              <div class="heroContent">
                <div>
                  ...
                  <p>${user.fullName}, você recebeu ${formatCurrency(received)}, pagou ${paidBills.length} conta(s) e ainda possui ${formatCurrency(free)} livre para decidir. O principal ponto de atenção continua sendo ${overdueBills.length} conta(s) atrasada(s).</p>
                </div>
                <div class="scoreBox">
                  <div class="label" style="color:rgba(255,255,255,.62)">North Score</div>
                  <strong>${northScore}</strong>
                  <span>+7 pontos no período</span>
                </div>
              </div>
            </section>
            <section class="grid">
              ${[
                ...
                ["Pendentes", String(pendingBills.length)],
            ...
            <section class="grid">
              ${[["Contas", paid],["Reserva", saved],["Metas", 300],["Livre", free]].map(([label, value]) => `<div class="card"><div class="label">${label}</div><div class="value">${formatCurrency(value as number)}</div></div>`).join("")}
            </section>
            <h2>Metas principais</h2>
            <p>${goals.slice(0, 3).map((goal) => `${goal.name}: ${Math.round((goal.current / goal.target) * 100)}%`).join(" • ") || "Nenhuma meta cadastrada."}</p>
            <section class="recommendation">
              <div class="label" style="color:#ffb08c">Recomendações North</div>
              <p>Priorize contas essenciais, revise o valor livre para decidir e mantenha a reserva em evolução. Existem ${overdueBills.length} conta(s) atrasada(s).</p>
            </section>
          </main>
        </body>
      </html>`;
...
function PlanningView({
  ...
  const [plannedIncome, setPlannedIncome] = useState(metrics.totalIncome || 6290);
  const [saved, setSaved] = useState(false);
  ...
  const [distribution, setDistribution] = useState([
    { id: "essenciais", label: "Essenciais", helper: "Contas fixas e moradia", value: 3145, icon: Home, tone: "orange" },
    { id: "alimentacao", label: "Alimentação", helper: "Mercado, restaurantes", value: 800, icon: ShoppingCart, tone: "green" },
    { id: "transporte", label: "Transporte", helper: "Combustível, apps, manutenção", value: 400, icon: Car, tone: "blue" },
    { id: "lazer", label: "Lazer", helper: "Diversão, hobbies, passeios", value: 250, icon: Heart, tone: "red" },
    { id: "reserva", label: "Reserva", helper: "Reserva de emergência", value: 500, icon: Shield, tone: "green" },
    { id: "metas", label: "Metas", helper: "Sonhos e objetivos", value: 300, icon: Target, tone: "purple" },
    { id: "atrasadas", label: "Contas atrasadas", helper: "Dívidas e pendências", value: 700, icon: Zap, tone: "red" },
  ]);
  ...
  const otherDistribution = Math.max(0, totalDistribution - fixedTotal - reserveTotal - totalGoals);
  const remaining = plannedIncome - fixedTotal - totalExpected - totalGoals - reserveTotal;
  ...
  const planningTips = remaining < 0
    ? [
      `Reduza ${formatCurrency(Math.abs(remaining))} para o plano fechar sem aperto.`,
    ...
    : [
      `Você ainda tem ${formatCurrency(remaining)} para decidir com calma.`,
    ...
    <div className="space-y-5">
      ...
      {remaining < 0 ? (
        <p className="rounded-3xl border border-red-500/15 bg-red-500/10 p-4 text-sm font-bold text-red-600">
          Você planejou {formatCurrency(Math.abs(remaining))} a mais do que espera receber.
        </p>
      ) : remaining > 0 ? (
        <p className="rounded-3xl border border-[#d75c27]/15 bg-[#d75c27]/10 p-4 text-sm font-bold text-[#b94d20]">
          Você ainda possui {formatCurrency(remaining)} livre para decidir.
        </p>
      ) : null}
        ...
        <div className="overflow-hidden rounded-[30px] border border-white/12 bg-[radial-gradient(circle_at_92%_12%,rgba(215,92,39,.34),transparent_18rem),linear-gradient(135deg,#211d19,#3a2a22)] p-6 text-white shadow-[0_18px_56px_rgba(33,29,25,.12)]">
          ...
          <p className="mt-3 max-w-2xl text-sm font-semibold leading-6 text-white/76">
            {remaining >= 0
              ...
              : "Os valores planejados passaram da receita prevista. Reduza lazer, metas ou despesas extras para fechar o mês com mais segurança."}
          </p>
...
export default function ReveeNorthApp() {
  ...
  const applyCloudState = (state: Partial<ReveeNorthCloudState>) => {
    ...
    if (state.security) setSecurity(state.security);
    ...
    if (typeof state.onboardingComplete === "boolean") setOnboardingComplete(state.onboardingComplete);
  };
  ...
  const data = useMemo<MonthData>(() => {
    ...
    return {
      selectedMonth,
      bills: bills.filter((bill) => monthKey(bill.dueDate) === selectedMonth),
      ...
      current.map((bill) =>
        bill.id === id && bill.status !== "paga"
          ? {
              ...bill,
              status: "paga",
              paidDate: getReferenceDate(selectedMonth),
              paidAmount: bill.expectedAmount,
              notes: bill.notes.includes("Registrado")
                ? bill.notes
                : `${bill.notes} • Registrado como pago`,
            }
          : bill,
      ),
    );
  ...
  const handleSaveBill = (updatedBill: Bill) => {
    setBills((current) =>
      current.map((bill) => (bill.id === updatedBill.id ? updatedBill : bill)),
    );
  };
